...
const CategoryController = {
    ...
    getCategoryByName : async (request : Request, response : Response ) => {
        try {
            const category = await CategoryRepository.getCategoryByName(request.params.name)
            return successResponseStatus(
                response,
    ...
    updateCategory : async (request : Request, response : Response) => {
        try {
            const {name, description } = request.body as CreateCategoryDto;
            const categoryIsExist = await CategoryRepository.getCategoryById(request.params.id)
